        ...
        <div className="space-y-6">
            <div className={`rounded-2xl border shadow-lg p-6 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200'}`}>
                ...
                {item.description ? (
                    ...
                    <p className={`italic ${isDark ? 'text-zinc-500' : 'text-gray-500'}`}>{tApp('AppDetails.info.noDescription', undefined, 'No description.')}</p>
                )}

                {item.longDescription && (
                    <div className={`mt-4 pt-4 border-t ${isDark ? 'border-zinc-800' : 'border-gray-100'}`}>
                        <h3 className={`text-sm font-semibold mb-2 ${isDark ? 'text-zinc-200' : 'text-gray-900'}`}>{tApp('AppDetails.info.detailedOverview', undefined, 'Detailed Overview')}</h3>
                ...
                {typeof item.price === 'number' && item.price > 0 && (
                    <div className={`mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm ${isDark
                        ...
                        {tApp('AppDetails.info.price', { price: Number(item.price).toFixed(2) }, `Price: €${Number(item.price).toFixed(2)}`)}
                    </div>
                )}
            ...
            {item.author && (
                <div className={`rounded-2xl border shadow-lg p-6 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200'}`}>
                    <h3 className={`text-sm font-semibold mb-3 ${isDark ? 'text-zinc-200' : 'text-gray-900'}`}>{tApp('AppDetails.info.creator', undefined, 'Creator')}</h3>
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className={`text-xs ${isDark ? 'text-zinc-500' : 'text-gray-500'}`}>{tApp('AppDetails.info.appDeveloper', undefined, 'App Developer')}</span>
                        </div>
                    </div>
                </div>
            )}
