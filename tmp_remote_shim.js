    ...
    const render = () => {
      try {
        ...
        panel.innerHTML = '<div style="font-size:18px;font-weight:600;margin-bottom:10px;">Otvorite aplikaciju kroz Thesara Play</div>' +
          '<div style="font-size:14px;line-height:1.6;margin-bottom:16px;">Ova verzija radi samo lokalno jer nema Thesara token. Pokrenite je preko Play linka kako bi se podaci sinkronizirali izmeÄ?u ureÄ?aja.</div>';
        const playUrl = (window.__THESARA_PLAY_URL__ || '').toString();
        if (playUrl) {
