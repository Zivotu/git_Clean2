...
async function applyCustomAssets(
  ...
  for (const asset of assets) {
    // Use the provided name as the relative path (e.g. "sprites/player.png")
    // Fallback to timestamp if name is missing
    const relativePath = asset.name || `asset-${Date.now()}`;

    // Basic security check to prevent directory traversal
    if (relativePath.includes('..')) {
      console.warn(`[bundle-worker] Skipping unsafe asset path: ${relativePath}`);
      continue;
    }

    const dest = path.join(targetDir, relativePath);

    // Ensure the parent directory for this specific asset exists
...
async function ensureAssetsInDist(
  ...
  for (const assetName of assetNames) {
    ...
    if (!existsInDist && await fileExists(publicPath)) {
      // Ensure parent directory exists in dist before copying
