    ...
    case 'checkout.session.completed': {
      ...
      if (userId) {
        ...
        try {
          ...
          if (userDoc.exists) {
            const userData = userDoc.data() as any; // Using 'any' for flexibility with new fields
            const referredBy = userData.referredBy;
            const ATTRIBUTION_WINDOW_DAYS = 60;

            if (referredBy && referredBy.redeemedAt) {
              const isWithinAttribution = (Date.now() - referredBy.redeemedAt) < (ATTRIBUTION_WINDOW_DAYS * 24 * 60 * 60 * 1000);

              if (isWithinAttribution) {
                ...
                const promoCodeRef = db.collection('promoCodes').doc(referredBy.promoCode);

                await db.runTransaction(async (t) => {
                  const FieldValue = (await import('firebase-admin/firestore')).FieldValue;
                  t.update(ambassadorRef, {
                    'ambassador.earnings.currentBalance': FieldValue.increment(commissionAmount),
                    'ambassador.earnings.totalEarned': FieldValue.increment(commissionAmount),
                  });
                  t.update(promoCodeRef, {
                    paidConversionsCount: FieldValue.increment(1),
                    totalRevenueGenerated: FieldValue.increment(amountTotal),
                  });
                });

                console.log(`[Ambassador] Commission of ${commissionAmount} processed for ${referredBy.ambassadorUid} from user ${userId}`);
              }
            }
          }
        } catch (error) {
          console.error('[Ambassador] Error processing commission:', error);
        }
